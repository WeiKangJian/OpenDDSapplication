...
int main(int argc, char *argv[])
{
	...
	try {
		...
		//注册类型支持类
		S03_TransferFile::FileMessageTypeSupport_var mts = new S03_TransferFile::FileMessageTypeSupportImpl;
		//支持类异常处理
		if (DDS::RETCODE_OK != mts->register_type(participant, "")) {
			std::cerr << "Failed to register the FileMessageTypeSupport." << std::endl;
			return 1;
		}
		//创建主题（同样也是FileMessage S03_TransferFile）
		...
		//类型窄化
		S03_TransferFile::FileMessageDataReader_var reader_i = S03_TransferFile::FileMessageDataReader::_narrow(dr);
		if (!reader_i)
		{
