...
int main(int argc, char *argv[])
{
	try {
		...
		//注册类型支持类
		YH02::M02TypeSupport_var mts = new YH02::M02TypeSupportImpl;
		//支持类异常处理
		if (DDS::RETCODE_OK != mts->register_type(participant, "")) {
			std::cerr << "Failed to register the M02TypeSupport." << std::endl;
			return 1;
		}
		//创建主题（同样也是M02 YH02）
		...
		//类型窄化
		YH02::M02DataReader_var reader_i = YH02::M02DataReader::_narrow(dr);
		if (!reader_i)
		{
